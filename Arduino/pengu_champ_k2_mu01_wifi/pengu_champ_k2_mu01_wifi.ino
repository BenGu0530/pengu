// pengu_champ_k2_mu01_wifi.ino — WiFi build of pengu_champ_k2_mu01.
//
// Identical gait, identical control law, identical gains. The only difference is that the
// webpage.ino / wireless.ino tabs are back, so the robot can be driven untethered from a
// browser at 192.168.4.1 (AP "PENGU") as well as from serial. Serial still wins when both
// send a command in the same loop, and the CSV telemetry still goes out over USB -- so if
// you want the data, stay plugged in and use the webpage only for the commands.
//
// Cost: joining the robot's AP takes the laptop off its own network.
//
// ---------------------------------------------------------------------------------------
// pengu_champ_k2_mu01.ino — the GRID-4 c6 champion (kappa=2, COM 1.31) on the
// ice surface, carrying every fix measured on 2026-08-28/29.
//
//   gait 1.67 / 340 / 95 / 24 / 20   at mu ~ 0.1   (the lab ice measures mu = 0.11)
//
// Differences from Arduino/pengu_champ (which is left untouched):
//   * kp = 0.5, not 2.0. Substituting T = A + s*J into the control law gives
//     d(cmd)/dJ = -kp, so the discrete loop evolves as dJ_{n+1} = dJ_n*[1-g(1+kp)] with
//     g the fraction of the gap the servo closes per period. Measured g ~= 0.67 on this
//     robot, which makes kp=2 marginal-to-divergent: the 2026-08-28 walk reversed the
//     torso command's sign on 77% of consecutive samples, railed the clamp, and tripped
//     the motor's overload protection (hwErr=32). kp=0.5 is the measured optimum
//     (g*(1+kp) ~= 1). The equilibrium J* = (kappa-1)*A does NOT depend on kp, so this is
//     the same control objective, only a different convergence rate -- and at kp=0.5 the
//     hardware reproduced the sim's ratios exactly: T/A 2.02 vs 2.02, J/A 1.03 vs 1.03.
//   * no delay(20). The inherited pacing delay sat on top of the real work, giving 25 Hz;
//     removing it and moving the telemetry-only IMU reads to 1 Hz gives 57-80 Hz. The gait
//     waveform is unchanged -- phase comes from millis(), absolute time.
//   * the integral uses the MEASURED period, not a hardcoded 0.02 s.
//   * TORSO_CLAMP_DEG = 25 (measured mechanical stop is +27.8/-31.0 at hip_off=20; the
//     sim command for this gait is +-22.1, so the clamp is close but not binding).
//   * serial only -- no WiFi tabs, so the laptop stays on its own network.
//
// TWO THINGS TO WATCH ON THIS GAIT (they do not apply to the slower 1.06 Hz test gait):
//   1. TORSO TORQUE. In sim this champion holds the torso at the 4.1 N.m stall limit for
//      13.2% of the walk -- by far the most demanding cell tested. A stable loop does not
//      remove that; it only stops the loop from adding its own thrashing on top. Watch
//      mA_torso in the telemetry: sustained 2000+ mA is the overload path. If it trips,
//      hwErr=32 and torque goes to 0; power-cycle to clear.
//   2. CRANK AMPLITUDE. pi*f*A = 498 deg/s demanded vs a measured ceiling of 420-440, so
//      the cranks deliver ~0.91 of the commanded 95 deg (measured at exactly this
//      freq/amp, hanging and loaded). Expect ~86 deg of actual stroke.
//
// Sim reference for this gait at mu=0.1 (grid5/walk_prediction.py, 10 s window):
//   speed 0.497 m/s | torso world roll rms 21.0 deg | A rms 11.4 | T/A 2.02
//   torso joint amplitude 38.1 deg | torso torque p95 4.10 N.m (stall), 13.2% of window
//
// Commands:  r = READY   w = WALK   q = IDLE   0 / 2 = kappa   t = CSV telemetry
//            4 / 5 / 6 / 7 = kp 2.0 / 1.0 / 0.5 / 0.25   s = S_TILT check   p = health
//            g = cycle the LEG motors' internal POSITION_P_GAIN 800/1200/1600/2000
// Default on boot: kappa = 2, kp = 0.5.
...
void loop() {
  ...
  switch (cmd) {
    ...
    case '0': case '2': {                 // A/B the torso law without touching anything else
      ...
      DEBUG_SERIAL.print(p_kappa > 1.0f ? "21.0" : "3.0");
      DEBUG_SERIAL.println(" deg at mu=0.1)");
      break;
    }
    case 's': {
