// pengu_k0_com105_mu05_v2_wifi.ino — WiFi build; every command has a button at
// 192.168.4.1.
//
// ---------------------------------------------------------------------------------------
// pengu_k0_com105_mu05_v2.ino — GRID-4 config c1 (kappa=0), COM 1.05, mu = 0.5.
// Second attempt after the first three gaits failed on hardware.
//
...
//
// What failed and why this set is different. The first attempt flashed the sweep's rank 1
// (hip_off 20) plus two runner-ups at hip_off 10, and all three sat backwards. The
// firmware's own note says HIP_REST_DEG is 10 "or the robot tips backward", so at
// hip_off 10 the walk adds no forward lean at all over the rest pose. Every gait here
// leans at least 25 deg.
//
//   'n'  1.95 /  0 / 125 / 28 / 25   the sweep champion, lean hardcoded from 20 to 25
//   'm'  1.99 / 30 / 125 / 28 / 30   the champion among cells with hip_off >= 30
//   'j'  1.99 / 30 / 125 / 28 / 35   the same cell, lean hardcoded from 30 to 35
//
// Only the lean is hardcoded off the sweep grid (which samples hip_off in 10 deg steps);
// freq, phase and both amplitudes are exactly the cells the sweep selected. This is a
// gait set from the co-design table being tested, not a controller being tuned, so
// kappa stays 0 and the configuration stays c1.
//
// Sweep values for the two cells (GRID-4, mu=0.5):
//
//   1.95 /  0 / 125 / 28 / 20   net_fwd 0.3500   neighbourhood 1.00
//   1.99 / 30 / 125 / 28 / 30   net_fwd 0.3101   neighbourhood 0.75
//
// Two things to expect, so they are not mistaken for new failures:
//
// 1. CRANK VELOCITY. Both cells sit at leg_amp 125 and freq ~2, so pi*f*A is 766 and
//    781 deg/s against a ceiling measured on this robot of ~430. The cranks can deliver
//    at most ~55% of the commanded 125 deg, i.e. about 70 deg. There is no cell in this
//    config, at mu=0.5, with hip_off >= 30, that both ranks well and fits the envelope --
//    the best that fits is 1.32/210/85/24/50 at net_fwd 0.1156, and its neighbourhood
//    score is 0.00, meaning every neighbouring cell falls. That is a property of the
//    config, not a selection mistake.
// 2. TORSO TRAVEL. kappa=0 asks for a large torso swing on this body and TORSO_CLAMP_DEG
//    is 25, against a stop measured at +30.9 deg (hips 0) and +27.8 (hips 20). The
//    command will be clipped on all three.
//
...
float p_hipOffD  = 25.0f;   // champion's 20 + 5 hardcoded                     // [deg] symmetric forward-pitch offset
...
void loop() {
  ...
  switch (cmd) {
    ...
    case 'n': case 'm': case 'j': {   // switch gait without reflashing
      if (cmd == 'n') {        // (1) the sweep champion, lean hardcoded +5 (20 -> 25)
        p_legFreq = 1.95f; p_legAmp = 125.0f*PI/180.0f; p_hipAmp = 28.0f*PI/180.0f;
        p_hipPhiD =   0.0f; p_hipOffD = 25.0f;
      } else if (cmd == 'm') { // (2) the champion among cells with hip_off >= 30
        p_legFreq = 1.99f; p_legAmp = 125.0f*PI/180.0f; p_hipAmp = 28.0f*PI/180.0f;
        p_hipPhiD =  30.0f; p_hipOffD = 30.0f;
      } else {                 // (3) same, +5 hardcoded on top (30 -> 35)
        p_legFreq = 1.99f; p_legAmp = 125.0f*PI/180.0f; p_hipAmp = 28.0f*PI/180.0f;
        p_hipPhiD =  30.0f; p_hipOffD = 35.0f;
      }
      torso_iErr = 0;
