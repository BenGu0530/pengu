...
/* added with the extra command buttons */
.row{display:flex;gap:8px;margin:6px 0}
.mini{flex:1;padding:12px 6px;border:none;border-radius:10px;font-size:.85rem;
      font-weight:600;background:#37474f;color:#fff;line-height:1.25}
.mini:active{transform:scale(.95);opacity:.8}
.mini small{font-weight:400;opacity:.75;font-size:.72rem}
.mini.gait.sel{background:#1d6fa4;box-shadow:0 0 10px #2196f366}
.btn-stop{background:#7f2f2f}
.warn{background:#8a5a2b}
.lbl{margin:14px 0 2px;font-size:.78rem;letter-spacing:.08em;text-transform:uppercase;opacity:.6}
/* control buttons */
...
<div class="ctrl">
  ...
  <!-- everything below is new: the sketch has far more commands than READY/WALK, and
       over WiFi the only way to reach them was to hand-type /cmd?key=X -->
  <div class="row">
    <button class="mini btn-stop"  onclick="send('q')">STOP</button>
    <button class="mini"           onclick="send('t')">telemetry</button>
    <button class="mini"           onclick="send('p')">health</button>
  </div>

  <div class="lbl">gait</div>
  <div class="row">
    <button class="mini gait" id="g-n" onclick="pick('n')">n &middot; 1.95/25&deg;<br><small>champ +5</small></button>
    <button class="mini gait" id="g-m" onclick="pick('m')">m &middot; 1.99/30&deg;<br><small>off&ge;30 champ</small></button>
    <button class="mini gait" id="g-j" onclick="pick('j')">j &middot; 1.99/35&deg;<br><small>off&ge;30 +5</small></button>
  </div>

  <div class="lbl">kappa</div>
  <div class="row">
    <button class="mini" onclick="send('0')">&kappa; = 0</button>
    <button class="mini" onclick="send('2')">&kappa; = 2</button>
  </div>

  <div class="lbl">kp &nbsp;<small>(0.5 is the measured optimum; 2.0 diverges on this robot)</small></div>
  <div class="row">
    <button class="mini" onclick="send('7')">0.25</button>
    <button class="mini" onclick="send('6')">0.5</button>
    <button class="mini" onclick="send('5')">1.0</button>
    <button class="mini warn" onclick="send('4')">2.0</button>
  </div>

  <div class="lbl">leg P gain</div>
  <div class="row">
    <button class="mini" onclick="send('g')">cycle 800 &rarr; 2000</button>
  </div>
</div>
...
function pick(g){
  send(g);
  ['n','m','j'].forEach(function(x){
    document.getElementById('g-'+x).classList.toggle('sel', x === g);
  });
}
function send(cmd) {
  fetch('/cmd?key=' + cmd)
