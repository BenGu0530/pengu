// pengu_kappa_ab.ino — one gait, two torso laws, switched from the keyboard.
//
// Purpose: measure the sim2real gap of the kappa torso controller, not to walk fast.
// The speed champions are knife edges (c6's 1.67/95 walks 0.379 m/s while 1.66/95 and
// 1.67/85 both fall), so a hardware-vs-sim difference there is swamped by which
// neighbouring cell the robot actually landed in. This gait was picked the opposite way:
// slow, with most of its 5-D grid neighbours also walking, at the mu closest to a lab
// floor.
//
//   gait 1.06 / 220 / 105 / 28 / 10   at mu = 0.3   (GRID-4, both c3 and c6 pass here)
//
// Sim prediction for a 10 s measured window (grid5/walk_prediction.py):
//
//                      kappa=2      kappa=0
//   speed              0.172        0.062  m/s
//   torso world roll   12.1         0.8    deg rms   <-- the discriminator, 15x apart
//   torso joint amp    24.4         16.4   deg
//   torso torque p95   1.81         0.61   N.m       <-- never near the 4.1 stall, so this
//   crank amplitude    104.2        104.5  deg           tests the control law, not the motor
//   hip amplitude      26.7/28.4    25.5/27.3 deg
//
// The legs are identical under both laws (crank 104.2 vs 104.5), so pressing '0' or '2'
// changes exactly one thing. Same upload, same boot zero, same battery, same floor.
//
// Serial only -- no WiFi tabs, so the laptop stays on its own network.
//
// Commands:  r = READY   w = WALK   q = IDLE   0 / 2 = set kappa   t = CSV telemetry
//            s = S_TILT sign check   p = motor health
//            4 / 5 / 6 / 7 = kp 2.0 / 1.0 / 0.5 / 0.25   (live, resets the integral)
//
// Torso clamp is 25 deg: the measured mechanical stop on this robot is +30.9/-31.9 at
// hip_off=0 and +27.8/-31.0 at hip_off=20 (torso_rom, 2026-08-28). kappa=2 asks for
// +-12.7 here, so the clamp is not binding.
...
void run_walk() {
  ...
  if (millis() - dbg > 1000) { dbg = millis();
    ...
    loop_count = 0;
    DEBUG_SERIAL.print("  step gain (g=1) = "); DEBUG_SERIAL.println(p_kp, 2);
  }
}
...
void loop() {
  ...
  switch (cmd) {
    ...
    case '0': case '2': {                 // A/B the torso law without touching anything else
      ...
      DEBUG_SERIAL.print(p_kappa > 1.0f ? "12.1" : "0.8");
      DEBUG_SERIAL.println(" deg, speed 0.172 / 0.062 m/s at mu=0.3)");
      break;
    }
    case 's': {
