// pengu_k0_com105_mu05_wifi.ino — WiFi build. Every command has a button on the page at
// 192.168.4.1 (AP "PENGU"); serial wins if both arrive in the same loop.
//
// ---------------------------------------------------------------------------------------
// pengu_k0_com105_mu05.ino — kappa=0 (Gait 1, torso held level) on the COM 1.05 body,
// gaits picked for mu = 0.5. GRID-4 config c1.
//
// >>> HARDWARE CHANGE REQUIRED <<<  COM ratio 1.05 = counterweight slid DOWN ~86 mm along
// the torso mast from the 1.31 build (sim slide -86.05 mm, mass held at 2.2724 kg).
//
//   'n'  1.95 /   0 / 125 / 28 / 20   (boot default)  sweep rank 1,   net 0.3500, nbhd 1.00
//   'm'  1.92 / 340 / 115 / 28 / 10                   net 0.2985, nbhd 1.00
//   'j'  1.63 / 290 /  85 / 28 / 10                   net 0.1303, nbhd 0.86, crank FITS
//
// This surface is where the sweep pays off -- mu=0.5 gaits reach 0.33-0.35 m/s against
// 0.17 at mu=0.1 -- but it is also where the crank demand is worst, because the fast cells
// all sit at leg_amp 115-125 and freq 1.9+.
//
//   gait   crank demand   deliverable at ~430 deg/s   sim speed as commanded / as executed
//   'n'    766 deg/s      70 of 125 deg (56%)         0.332  ->  0.135 m/s
//   'm'    694 deg/s      71 of 115 deg (62%)         0.305  ->  0.099 m/s
//   'j'    435 deg/s      85 of  85 deg (full)        0.131  ->  0.131 m/s
//
// So the sweep's top two are predicted to end up SLOWER than the one that ranks 40x lower,
// purely because the motors cannot turn fast enough. Pressing n, then m, then j in one
// session tests that directly.
//
// TORSO CLAMP. kappa=0 asks for a large torso swing on this body -- +-43.3 deg for 'n',
// +-35.1 for 'm', +-22.9 for 'j' -- while the measured mechanical stop is +30.9/-31.9 deg
// at hip_off=0 and +27.8/-31.0 at hip_off=20. TORSO_CLAMP_DEG is 25 here, so 'n' and 'm'
// WILL be clipped; only 'j' fits inside the travel. That is a deliberate deviation from
// the simulation, taken instead of driving the torso into its hard stop.
//
// Sim reference, mu=0.5, kappa=0, COM 1.05, 10 s window:
//
//                        n (1.95)   m (1.92)   j (1.63)
//   speed                0.332      0.305      0.131  m/s
//   torso world roll     4.6        3.7        2.2    deg rms   (kappa=0 holds it level)
//   torso command amp    86.6       70.1       45.8   deg       (clamped to +-25)
//   torso torque p95     1.60       1.86       1.34   N.m       (stall 4.1)
//   crank torque p95     2.65       2.22       0.95   N.m
//
// Everything else inherited: kp = 0.5, no pacing delay (57-80 Hz control), integral on the
// measured period, leg POSITION_P_GAIN booting at 1600.
//
// Commands:  r = READY   w = WALK   q = IDLE   0 / 2 = kappa   n / m / j = gait
//            t = telemetry   4 / 5 / 6 / 7 = kp 2.0 / 1.0 / 0.5 / 0.25   g = leg P gain
//            p = health.  Over WiFi every one of these has a button on the page.
...
void loop() {
  ...
  switch (cmd) {
    ...
    case 'n': case 'm': case 'j': {   // switch gait without reflashing
      if (cmd == 'n') {        // sweep rank 1; 766 deg/s of crank demand
        p_legFreq = 1.95f; p_legAmp = 125.0f*PI/180.0f; p_hipAmp = 28.0f*PI/180.0f;
        p_hipPhiD =   0.0f; p_hipOffD = 20.0f;
      } else if (cmd == 'm') { // 85% of the speed for 9% less crank demand
        p_legFreq = 1.92f; p_legAmp = 115.0f*PI/180.0f; p_hipAmp = 28.0f*PI/180.0f;
        p_hipPhiD = 340.0f; p_hipOffD = 10.0f;
      } else {                 // 'j': the only one whose crank demand fits the envelope
        p_legFreq = 1.63f; p_legAmp =  85.0f*PI/180.0f; p_hipAmp = 28.0f*PI/180.0f;
        p_hipPhiD = 290.0f; p_hipOffD = 10.0f;
      }
      torso_iErr = 0;
      ...
      DEBUG_SERIAL.println(cmd == 'n' ? "43.3 deg (clipped)" : (cmd == 'm' ? "35.1 deg (clipped)" : "22.9 deg"));
      break;
    }
    case '0': case '2': {                 // A/B the torso law without touching anything else
      ...
      DEBUG_SERIAL.print(p_kappa > 1.0f ? "-" : "4.6");
      DEBUG_SERIAL.println(" deg at mu=0.5, COM 1.05, kappa=0)");
      break;
    }
    case 's': {
