// pengu_k0_com105_mu01_wifi.ino — WiFi build; commands from a browser at 192.168.4.1
// (AP "PENGU") as well as serial, serial wins. Telemetry still leaves over USB only.
//
// ---------------------------------------------------------------------------------------
// pengu_k0_com105_mu01.ino — kappa=0 (Gait 1, torso held level) on the COM 1.05 body,
// gait picked for mu = 0.1. This is GRID-4 config c1.
//
// >>> HARDWARE CHANGE REQUIRED <<<  COM ratio 1.05 means the counterweight is slid DOWN
// about 86 mm along the torso mast from the 1.31 build (sim slide -86.05 mm, mass held at
// 2.2724 kg).
//
//   'n'  1.80 / 260 / 125 / 20 / 50   (boot default)  sweep rank 1,   net 0.1703
//   'm'  1.65 / 280 / 125 / 28 / 30                   best at hip_off 30, net 0.1260
//   'j'  1.10 / 280 / 115 / 28 / 10                   crank fits the envelope, net 0.0886
//
// net_fwd rises monotonically with hip_off on this body (0.089 at 10 deg, 0.126 at 30,
// 0.170 at 50), so the lean is buying the speed. 'm' gives up 26% of the speed for 20 deg
// less lean, which buys back torso travel: the stop is ~+26 deg at hip_off 30 against
// ~+23 at 50, while the torso command drops from +-29.5 to +-24.6. 'j' is the only one
// the cranks can execute in full (397 deg/s against the ~430 ceiling) -- in simulation it
// is the slowest, but at the amplitude the motors will actually deliver:
//
//     'n' commanded 0.170 m/s  ->  0.015 at the 76 deg the cranks can reach
//     'm' commanded 0.131 m/s  ->  0.053 at 83 deg
//     'j' commanded 0.087 m/s  ->  0.087, delivered in full
//
// TWO PREDICTIONS TO CHECK, because the default gait is expected to struggle:
//
// 1. The crank demand of 'n' is pi*f*A = 707 deg/s against a ceiling measured on this
//    robot of ~430 deg/s, so the cranks can deliver at most ~61% of the commanded 125 deg.
//    Re-running the sim at that reduced amplitude gives:
//
//        1.80 / 260 / 125 / 20 / 50   ->  0.170 m/s      (as commanded)
//        1.80 / 260 /  76 / 20 / 50   ->  0.015 m/s      (as the motors will execute it)
//        1.02 / 260 / 125 / 24 / 50   ->  0.126 m/s      ('m', crank 401, delivered in full)
//
//    So the sweep's best gait is predicted to nearly stop on hardware while the slower
//    reachable one keeps most of its speed. Running both in one session is a direct test
//    of whether the velocity envelope matters -- press 'n', then 'm', same battery, same
//    floor.
//
// 2. hip_off = 50 deg is a large forward lean and it narrows the torso's travel. The stop
//    was measured at +30.9 deg with hips at 0 and +27.8 deg with hips at 20, i.e. about
//    -0.155 deg per degree of hip offset, which extrapolates to roughly +23 deg at
//    hip_off = 50. Both gaits ask the torso for +-28 to +-29.5 deg, so TORSO_CLAMP_DEG is
//    set to 20 here rather than 25 and the command WILL be clipped. That is a deliberate
//    deviation from the simulation, taken to avoid driving the torso into its hard stop.
//    MEASURE THE REAL STOP AT hip_off = 50 with Arduino/torso_rom before trusting the
//    extrapolation.
//
// Sim reference, mu=0.1, kappa=0, COM 1.05, 10 s window:
//
//                        n (1.80)     m (1.02)
//   speed                0.170        0.126  m/s
//   torso world roll     4.0          2.0    deg rms   (kappa=0 holds the torso level)
//   hip-axis roll A      18.8         16.1   deg rms
//   torso joint amp      51.5         50.4   deg
//   torso command amp    58.9         56.8   deg       (clamped to +-20 on hardware)
//   torso torque p95     1.18         0.45   N.m       (stall 4.1, never approached)
//   crank torque p95     1.98         0.62   N.m
//
// kappa=0 is far less sensitive to errors in the reconstructed A than kappa=2:
// d(cmd)/dA = (kappa-1) + kp*kappa = 1 here, against 5 at kappa=2.
//
// Everything else is inherited: kp = 0.5, no pacing delay (57-80 Hz control), integral on
// the measured period, leg POSITION_P_GAIN booting at 1600.
//
// Commands:  r = READY   w = WALK   q = IDLE   0 / 2 = kappa   n / m = gait   t = telemetry
//            4 / 5 / 6 / 7 = kp 2.0 / 1.0 / 0.5 / 0.25   g = leg P gain   p = health
...
const float TORSO_CLAMP_DEG = 20.0f;   // measured mechanical stop: +30.9/-31.9 at hip_off=0,
...
void loop() {
  ...
  switch (cmd) {
    ...
    case 'n': case 'm': case 'j': {   // switch gait without reflashing
      if (cmd == 'n') {        // sweep rank 1; needs hip_off 50 and 707 deg/s of crank
        p_legFreq = 1.80f; p_legAmp = 125.0f*PI/180.0f; p_hipAmp = 20.0f*PI/180.0f;
        p_hipPhiD = 260.0f; p_hipOffD = 50.0f;
      } else if (cmd == 'm') { // best at hip_off 30: 74% of the speed, 20 deg less lean
        p_legFreq = 1.65f; p_legAmp = 125.0f*PI/180.0f; p_hipAmp = 28.0f*PI/180.0f;
        p_hipPhiD = 280.0f; p_hipOffD = 30.0f;
      } else {                 // 'j': the only one whose crank demand fits the envelope
        p_legFreq = 1.10f; p_legAmp = 115.0f*PI/180.0f; p_hipAmp = 28.0f*PI/180.0f;
        p_hipPhiD = 280.0f; p_hipOffD = 10.0f;
      }
      torso_iErr = 0;
      ...
      DEBUG_SERIAL.println(cmd == 'n' ? "29.5 deg" : (cmd == 'm' ? "24.6 deg" : "14.6 deg"));
      break;
    }
    case '0': case '2': {                 // A/B the torso law without touching anything else
      ...
      DEBUG_SERIAL.print(p_kappa > 1.0f ? "-" : "4.0");
      DEBUG_SERIAL.println(" deg at mu=0.1, COM 1.05, kappa=0)");
      break;
    }
    case 's': {
