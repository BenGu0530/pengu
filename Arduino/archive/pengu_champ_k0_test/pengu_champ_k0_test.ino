// pengu_champ_k0_test.ino — SERIAL-ONLY test build. Same gait and same control law as
// pengu_champ_k0, with two additions for the sim-vs-hardware comparison:
//   * TORSO_CLAMP_DEG 45 -> 25 (measured mechanical stop is +27.8/-31.0 deg at
//     hip_off=20; torso_rom, 2026-08-28)
//   * a 20 Hz CSV telemetry stream on 't' (goal vs present for all five motors,
//     torso current, IMU roll/pitch, the reconstructed axis, and the loop period)
// The WiFi tabs are deliberately absent: the AP would take the laptop off its own
// network. Everything is on USB serial. The original sketch is left untouched.
//
// (from) pengu_champ_k0.ino — GRID-4 c3 champion gait (COM-1.31 body, Gait 1: kappa=0,// torso held WORLD-UPRIGHT by feedback), K=5-verified in sim at mu=0.1.
...
void setup() {
  ...
  DEBUG_SERIAL.println("pengu_champ_k0 (Gait 1) ready. r=Ready w=Walk q=Idle s=SignCheck (serial only)");
