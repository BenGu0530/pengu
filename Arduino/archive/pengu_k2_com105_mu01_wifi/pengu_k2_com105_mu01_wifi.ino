// pengu_k2_com105_mu01_wifi.ino — WiFi build; commands from a browser at 192.168.4.1
// (AP "PENGU") as well as serial, serial wins. Telemetry still leaves over USB only.
//
// ---------------------------------------------------------------------------------------
// pengu_k2_com105_mu01.ino — kappa=2 on the COM 1.05 body, gait picked for mu = 0.1.
//
...
// 2.2724 kg). Flashing this onto the 1.31 robot runs the wrong gait on the wrong body.
//
//   'n'  1.27 / 350 / 105 / 28 / 20   (boot default)  sim net_fwd 0.167, neighbourhood 1.00
//   'm'  1.06 / 310 / 125 / 24 / 30                   sim net_fwd 0.240, neighbourhood 1.00
//
// Why not the fastest gait in the sweep. On this body the kappa=2 torso law asks for a far
// bigger torso swing than it does at COM 1.31, and the mechanical stop does not move:
//
//   sweep rank 1   1.05 / 40 / 125 / 28 / 50   net 0.254   torso command +-45.0 deg
//   'm'            1.06 / 310 / 125 / 24 / 30  net 0.240   torso command +-23.4 deg
//   'n'            1.27 / 350 / 105 / 28 / 20  net 0.167   torso command +-22.2 deg
//
// The measured stop on this robot is +30.9/-31.9 deg at hip_off=0 and +27.8/-31.0 at
// hip_off=20 (torso_rom, current-capped at 800 mA, position pinned with current at the
// cap). Rank 1 needs +-45 and is simply not reachable -- it saturates even the simulator's
// own +-45 clamp. 'm' needs +-23.4 at hip_off=30, where the stop is around +26, so it sits
// on the limit. 'n' needs +-22.2 at hip_off=20, where the stop is measured at +27.8: about
// 5.6 deg of margin, which is why it is the default.
//
// Crank demand pi*f*A = 419 deg/s for 'n' and 416 for 'm', both just inside the measured
// ceiling of ~430 deg/s, so expect close to full leg amplitude rather than the 0.865 the
// 1.67 Hz / 95 deg gait delivered.
//
// Sim reference, mu=0.1, 10 s window (grid5/walk_prediction.py):
//
//                        n (1.27)     m (1.06)
//   speed                0.167        0.240  m/s
//   torso world roll     26.9         29.5   deg rms
//   hip-axis roll A      14.9         16.4   deg rms
//   T/A                  1.81         1.80
//   torso joint amp      39.6         43.8   deg
//   torso torque p95     2.58         2.40   N.m   (stall 4.1)
//   torso >=90% stall    1.1%         1.0%   of the window
//
// Everything else is inherited from pengu_champ_k2_mu01: kp = 0.5 (kp=2 is above this
// robot's measured stability limit and trips the overload protection), no pacing delay
// (57-80 Hz control), integral on the measured period, TORSO_CLAMP_DEG = 25, leg
// POSITION_P_GAIN booting at 1600.
//
...
float p_legFreq  = 1.27f;                     // [Hz]
float p_legAmp   = 105.0f * PI / 180.0f;      // [rad] crank amplitude (unipolar)
float p_hipAmp   = 28.0f  * PI / 180.0f;      // [rad] hip half-rectified swing
float p_hipPhiD  = 350.0f;                    // [deg] hip-vs-leg phase offset
...
void loop() {
  ...
  switch (cmd) {
    ...
    case 'n': case 'm': {          // switch gait without reflashing
      if (cmd == 'n') { p_legFreq = 1.27f; p_legAmp = 105.0f*PI/180.0f; p_hipAmp = 28.0f*PI/180.0f;
                        p_hipPhiD = 350.0f; p_hipOffD = 20.0f; }
      else            { p_legFreq = 1.06f; p_legAmp = 125.0f*PI/180.0f; p_hipAmp = 24.0f*PI/180.0f;
                        p_hipPhiD = 310.0f; p_hipOffD = 30.0f; }
      torso_iErr = 0;
      ...
      DEBUG_SERIAL.println(cmd == 'n' ? "22.2 deg" : "23.4 deg (at the mechanical stop)");
      break;
    }
    case '0': case '2': {                 // A/B the torso law without touching anything else
      ...
      DEBUG_SERIAL.print(p_kappa > 1.0f ? "26.9" : "-");
      DEBUG_SERIAL.println(" deg at mu=0.1, COM 1.05)");
      break;
    }
    case 's': {
