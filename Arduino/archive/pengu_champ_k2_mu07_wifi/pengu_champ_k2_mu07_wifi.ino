// pengu_champ_k2_mu07_wifi.ino — WiFi build of pengu_champ_k2_mu07.
//
// Same gait, same control law, same gains; webpage.ino / wireless.ino tabs restored, so
// commands can come from a browser at 192.168.4.1 (AP "PENGU") as well as serial (serial
// wins). Telemetry still leaves over USB only.
//
// ---------------------------------------------------------------------------------------
// pengu_champ_k2_mu07.ino — the GRID-4 c6 rank-1 gait at mu = 0.7 (kappa=2, COM 1.31),
// carrying every fix measured on 2026-08-28/29.
//
//   'n'  NOMINAL (boot default)   1.88 / 240 / 115 / 16 / 10   sim net_fwd 0.115, nbhd 0.80
//   'm'  REACHABLE               1.22 / 220 /  85 / 20 / 10   sim net_fwd 0.109, nbhd 0.93
//
// The two are 6% apart in the sweep, and the reachable one has the better neighbourhood
// score of the whole mu=0.7 list. The nominal gait demands a peak crank rate of
// pi*f*A = 679 deg/s against a measured ceiling of 420-440, so it will execute at roughly
// 63% amplitude (leg_amp ~72 rather than 115) -- a different gait, not a degraded one.
// The 'm' gait needs 326 deg/s and fits, and it is the same cell that came out on top of
// the reachable set at mu=0.5, so it is worth carrying across surfaces.
//
// Sim reference, mu=0.7, 10 s window (grid5/walk_prediction.py):
//
//                        n (1.88)     m (1.22)
//   speed                0.116        0.108  m/s
//   torso world roll     8.9          8.3    deg rms
//   hip-axis roll A      4.9          4.1    deg rms
//   T/A                  1.82         2.02
//   torso command amp    19.8         12.8   deg  (clamp is 25)
//   torso torque p95     1.91         1.11   N.m  (stall 4.1; 0% of the window near it)
//   crank torque p95     1.71         0.49   N.m
//
// Unlike the mu=0.5 pair, both gaits here leave the torso plenty of roll to work with, so
// either one tests the kappa law. The nominal one costs 3.5x the crank torque.
//
// Everything else is inherited: kp = 0.5 (kp=2 is above this robot's measured stability
// limit and trips the overload protection), no pacing delay (57-80 Hz control), integral on
// the measured period, TORSO_CLAMP_DEG = 25 against a measured stop of +27.8/-31.0, and leg
...
float p_legFreq  = 1.88f;                     // [Hz]
float p_legAmp   = 115.0f * PI / 180.0f;      // [rad] crank amplitude (unipolar)
float p_hipAmp   = 16.0f  * PI / 180.0f;      // [rad] hip half-rectified swing
...
void loop() {
  ...
  switch (cmd) {
    ...
    case 'n': case 'm': {          // switch gait without reflashing
      if (cmd == 'n') { p_legFreq = 1.88f; p_legAmp = 115.0f*PI/180.0f; p_hipAmp = 16.0f*PI/180.0f;
                        p_hipPhiD = 240.0f; p_hipOffD = 10.0f; }
      else            { p_legFreq = 1.22f; p_legAmp =  85.0f*PI/180.0f; p_hipAmp = 20.0f*PI/180.0f;
    ...
    case '0': case '2': {                 // A/B the torso law without touching anything else
      ...
      DEBUG_SERIAL.print(p_kappa > 1.0f ? "8.9" : "-");
      DEBUG_SERIAL.println(" deg at mu=0.7, nominal gait)");
      break;
    }
    case 's': {
