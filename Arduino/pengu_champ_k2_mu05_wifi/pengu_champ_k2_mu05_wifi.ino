// pengu_champ_k2_mu05_wifi.ino — WiFi build of pengu_champ_k2_mu05.
//
// Same gait, same control law, same gains. The webpage.ino / wireless.ino tabs are back,
// so commands can come from a browser at 192.168.4.1 (AP "PENGU") as well as from serial;
// serial wins if both arrive in the same loop. The CSV telemetry still leaves over USB, so
// stay plugged in if you want the data and use the webpage only to drive it.
//
// ---------------------------------------------------------------------------------------
// pengu_champ_k2_mu05.ino — the GRID-4 c6 rank-1 gait at mu = 0.5 (kappa=2, COM 1.31),
// carrying every fix measured on 2026-08-28/29.
//
//   'n'  NOMINAL (boot default)   1.92 / 240 / 115 / 20 / 10   sim net_fwd 0.121, nbhd 0.80
//   'm'  REACHABLE               1.21 / 220 /  85 / 20 / 10   sim net_fwd 0.104, nbhd 0.87
//
// Why both are here. The rank-1 gait demands a peak crank rate of pi*f*A = 694 deg/s; the
// measured ceiling on this robot is 420-440 deg/s (and during walking the cranks delivered
// only 0.865 of a 95 deg command at 1.67 Hz, sitting exactly on that ceiling). So the
// nominal gait will execute at roughly 62% amplitude -- leg_amp ~71 deg rather than 115 --
// which is a different gait, not a degraded one. The 'm' gait needs 323 deg/s and fits.
//
// Sim reference, mu=0.5, 10 s window (grid5/walk_prediction.py):
//
//                        n (1.92)     m (1.21)
//   speed                0.137        0.097  m/s
//   torso world roll     3.6          8.7    deg rms
//   hip-axis roll A      1.6          4.3    deg rms
//   T/A                  2.06         2.02
//   torso command amp    9.6          13.2   deg  (clamp is 25)
//   torso torque p95     1.19         1.16   N.m  (stall 4.1; 0% of the window near it)
//
// Note what that table says about testing the kappa law: the nominal gait leaves A at
// 1.6 deg, so the torso has almost nothing to amplify and the measurement has little
// signal. The reachable gait gives 2.7x the A and 2.4x the torso roll.
//
// Everything else is inherited from pengu_champ_k2_mu01: kp = 0.5 (kp=2 is above this
// robot's measured stability limit and trips the motor's overload protection), no pacing
// delay (57-80 Hz control), integral on the measured period, TORSO_CLAMP_DEG = 25 against
// a measured mechanical stop of +27.8/-31.0, and leg POSITION_P_GAIN booting at 1600
// (measured strictly better than the 800 default: crank 0.835 -> 0.854, hip 0.964 -> 0.986,
// with no cost in current or temperature).
//
...
int      pgain_idx = 2;      // boot at 1600: measured strictly better than 800
...
void loop() {
  ...
  switch (cmd) {
    ...
    case 'n': case 'm': {          // switch gait without reflashing
      if (cmd == 'n') { p_legFreq = 1.92f; p_legAmp = 115.0f*PI/180.0f; p_hipAmp = 20.0f*PI/180.0f;
                        p_hipPhiD = 240.0f; p_hipOffD = 10.0f; }
      else            { p_legFreq = 1.21f; p_legAmp =  85.0f*PI/180.0f; p_hipAmp = 20.0f*PI/180.0f;
                        p_hipPhiD = 220.0f; p_hipOffD = 10.0f; }
      torso_iErr = 0;
      ...
      DEBUG_SERIAL.println(" deg/s (measured ceiling ~430)");
      break;
    }
    case 'g': {
    ...
    case '0': case '2': {                 // A/B the torso law without touching anything else
      ...
      DEBUG_SERIAL.print(p_kappa > 1.0f ? "3.6" : "-");
      DEBUG_SERIAL.println(" deg at mu=0.5, nominal gait)");
      break;
    }
    case 's': {
