...
# hips+torso only, w=1.0. Full 2x2.
# Usage: nohup bash rl_grid4/run_e2x2_hf3.sh >> rl_grid4/runs/e2x2hf4.log 2>&1 &
set -u
cd "$(dirname "$0")/.."
...
run_cell () {
  ...
  echo "=== e2x2hf4 $cell (single-stage, r3d) $(date) ==="
  nice -n 10 "$PY" rl_grid4/train_grid4.py --mode e2 --seed "$S" --n-envs 8 \
    --curriculum --name "e2x2hf4/$cell" "$@"
  echo "=== e2x2hf4 $cell eval $(date) ==="
  "$PY" rl_grid4/eval_grid4_policy.py "rl_grid4/runs/e2x2hf4/$cell/ckpts/final.zip" \
