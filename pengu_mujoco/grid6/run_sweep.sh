...
#
#   bash grid6/run_sweep.sh c1         # c1..c6 = GRID-4 configs, c7..c10 = COM 1.10/1.40
#   bash grid6/run_sweep.sh c10 8      # config c10, force 8 shards (default = cores-2)
#   GRID3_PY=/path/to/python bash grid6/run_sweep.sh c1   # use a specific python
#
...
JOB="${1:-c1}"
case "$JOB" in
  c[1-9]|c10)  export CONFIG="$JOB" ; TAGN="grid6_$JOB" ; SCRIPT=grid6/grid6_sweep.py
               # grid5-v2: map is deterministic, K fixed at 1 in grid6_sweep.py (no DR_K)
               CSV="results/gait_sweep/sweep_grid5_${JOB}_freq_hip_phi_leg_amp_hip_amp_hip_off_mu.csv" ;;
  *) echo "usage: bash grid6/run_sweep.sh [c1..c10] [n_shards]"; exit 2 ;;
esac
