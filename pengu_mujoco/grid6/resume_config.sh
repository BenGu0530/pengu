...
#
#   bash grid6/resume_config.sh c1                # plain
#   SWEEP_NICE=19 bash grid6/resume_config.sh c1  # shared box (children inherit nice)
set -u
...
nice -n "${SWEEP_NICE:-0}" bash grid6/run_sweep.sh "$CFG"
echo "== arming watchdog =="
SWEEP_NICE="${SWEEP_NICE:-0}" bash grid6/sweep_watchdog.sh install "$CFG"
