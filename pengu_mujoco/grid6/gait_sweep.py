...
# APPEND, never insert(0, ...): the parent tree holds same-named twins of gait_config,
# torso_control and friction_utils, and prepending it makes every bare import below
# resolve to the ROOT copy instead of this tree's. grid5 has this bug and silently runs
# the root torso_control; here the tree's own modules must win.
...
# bit-identical 3-vector helpers; np.cross's generic path was 83% of a trial
    ...
    while data.time < end_t:
        ...
        for c in range(data.ncon):
            ...
            if ft:
                ...
                v_pt = vf6[3:6] + cross3(vf6[0:3], ct.pos - data.geom_xpos[fg])
                n = ct.frame[0:3]
                v_tan = v_pt - np.dot(v_pt, n) * n
                slipnum[ft] += fn * norm3(v_tan)
                if EXTENDED_METRICS:
                    con_pts[ft].append((fn, math.hypot(f6[1], f6[2]),
                                        norm3(v_tan), ct.pos[:2].copy()))
                    omega_foot[ft] = norm3(vf6[0:3])
        for s in ("L", "R"):
        ...
        if meas_prev is not None:
            ...
            if EXTENDED_METRICS:
                ...
                nf2 = float(np.linalg.norm(fvec))   # fvec is 2-D; norm3 is 3-D only
                if nf2 > 1e-9:
                    fhat = fvec / nf2
