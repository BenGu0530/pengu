...
# reference grid6/sweep_watchdog.sh so grid4 watchdog lines are never disturbed.
#
#   bash grid6/sweep_watchdog.sh c5 [n_shards]        # one-shot check (safe from cron)
#   bash grid6/sweep_watchdog.sh install c5 [n]       # add @reboot + every-10-min crontab
#
...
if [ "${1:-}" = "install" ]; then
  ...
  L1="@reboot sleep 90 && cd '$REPO' && ${NPFX}bash grid6/sweep_watchdog.sh $CFG $N >> results/gait_sweep/watchdog5.log 2>&1"
  L2="*/10 * * * * cd '$REPO' && ${NPFX}bash grid6/sweep_watchdog.sh $CFG $N >> results/gait_sweep/watchdog5.log 2>&1"
  ( crontab -l 2>/dev/null | grep -v 'grid6/sweep_watchdog' ; echo "$L1"; echo "$L2" ) | crontab -
  echo "grid5 watchdog installed for $CFG:"; crontab -l | grep 'grid6/sweep_watchdog'
  exit 0
fi
...
LIVE=$(pgrep -f 'grid6_sweep[.]py' | wc -l | tr -d ' ')
if [ "$LIVE" -lt "$N" ]; then
  ...
  pkill -f 'grid6_sweep[.]py' 2>/dev/null; sleep 2
  GRID3_PY="${GRID3_PY:-$REPO/.sweep_venv/bin/python}" nice -n "${SWEEP_NICE:-0}" bash grid6/run_sweep.sh "$CFG" "$N"
else
