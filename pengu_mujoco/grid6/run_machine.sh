...
#
#   nohup bash grid6/run_machine.sh <machine> > results/gait_sweep/machine_$(hostname).log 2>&1 &
...
NICE=0; NSH="${2:-}"                                      # optional 2nd arg: shard count (default cores-2)
case "$M" in
...
for CFG in $QUEUE; do
  ...
  SWEEP_NICE=$NICE bash grid6/sweep_watchdog.sh install "$CFG" $NSH >/dev/null 2>&1 || true
  nice -n "$NICE" bash grid6/run_sweep.sh "$CFG" $NSH
  while [ ! -f "$CSV.done" ]; do
    sleep 300
