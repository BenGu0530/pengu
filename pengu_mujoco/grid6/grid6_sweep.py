...
Differences from GRID-4 (physics/grid4_sweep.py, protocol a22f80b):
  ...
  AXES     freq 1.21-2.00 (GRID-4 top-20 had no champion below 1.21 — a physics
           ...
           mu {0.1,0.3,0.5,0.7}; c6 ONLY adds 0.9 (v2.4 high-grip probe).
...
usage (run from grid5/ so the local module copies win):
  CONFIG=c1 python grid6_sweep.py count|initcsv
  CONFIG=c1 N_SHARDS=10 SHARD_ID=3 python grid6_sweep.py
...
# GRID-6 runs the HARDWARE MODEL: models/hardware_c1, a CAD export of the physical
# lowered-counterweight robot, hardened to the 5-actuator convention and ballasted to the
# ladder's 2.2724 kg. Its COM ratio (1.0500) is a property of the geometry, so unlike
# GRID-5 nothing is slid at load time -- the ratio is VERIFIED, not tuned. That is the
# whole point: same mass, same ratio, but the torso's rotational inertia is 1.76x / 1.46x
# / 5.97x the slide-tuned variant's, because splitting the mass physically (structure
# high, counterweight low) spreads it about its own centre while a slide does not.
os.environ["PENGU_MODEL"] = os.environ.get("PENGU_MODEL", "hardware_c1")
_HERE = os.path.dirname(os.path.abspath(__file__)); _ROOT = os.path.dirname(_HERE)
sys.path.insert(0, _HERE)                   # grid6 copies FIRST (never grid5 or the root)
...
assert "grid6" in os.path.abspath(gs.__file__), gs.__file__
assert "grid6" in os.path.abspath(gc.__file__), gc.__file__
import torso_control as _tc
assert "grid6" in os.path.abspath(_tc.__file__), _tc.__file__
    # gait_sweep.py prepends the PARENT dir to sys.path, so a bare import after
    # that line silently picks the root-level twin. These two asserts are the
    # only thing standing between "grid6 ran" and "grid6 ran grid5's physics".
# GRID-5 pinned every config to the one base model because the COM ladder was built by
# sliding it. GRID-6 runs a genuinely different geometry, so the guard becomes a whitelist:
# still refuse anything unexpected, but allow the as-built export.
assert gc.XML_PATH.endswith(("pengu1_31/scene.xml", "hardware_c1/scene.xml")), gc.XML_PATH
...
if SMOKE:
    ...
    if CONFIG == "c6":
        # v2.4 (Ben 2026-08-30): mu=0.9 high-grip probe on c6 ONLY; other configs
        # stay on the 4-point mu axis. Floor priority hack makes this the contact mu.
...
# ---------------------------------------------------------------- targeted cell lists
# GRID6_CELLS points at a CSV of freq,hip_phi,leg_amp,hip_amp,hip_off. When set, the
# sweep visits exactly those cells (times every mu) instead of the full cross product.
# Two uses: stage 1 scans c1's robust region at coarse frequency, stage 2 refines the
# survivors at 0.01. Both are ordinary grid6 rows -- same protocol, same columns, same
# determinism -- they just do not enumerate the whole lattice.
CELL_LIST = os.environ.get("GRID6_CELLS", "")
CELLS_EXPLICIT = None
if CELL_LIST:
    with open(CELL_LIST) as _f:
        _rd = csv.reader(_f); next(_rd)
        CELLS_EXPLICIT = [tuple(round(float(x), 4) for x in r[:5]) for r in _rd if r]
    assert CELLS_EXPLICIT, f"empty cell list: {CELL_LIST}"
    # the axes are still declared so the manifest records what was covered
    FREQS    = np.array(sorted({c[0] for c in CELLS_EXPLICIT}))
    HIP_PHIS = np.array(sorted({c[1] for c in CELLS_EXPLICIT}))
    LEG_AMPS = np.array(sorted({c[2] for c in CELLS_EXPLICIT}))
    HIP_AMPS = np.array(sorted({c[3] for c in CELLS_EXPLICIT}))
    HIP_OFFS = np.array(sorted({c[4] for c in CELLS_EXPLICIT}))
    _CELL_IX = {c: i for i, c in enumerate(CELLS_EXPLICIT)}

TAG = (f"grid6_{CONFIG}" + ("_smoke" if SMOKE else "")
       + (("_" + os.path.basename(CELL_LIST).replace("cells_", "").replace(".csv", ""))
          if CELL_LIST else ""))   # NOT grid5_: the c1 map from PSC/XPS uses that name and
                          # this is a different model, not more rows for the same map
...
def cells():
    if CELLS_EXPLICIT is not None:
        yield from CELLS_EXPLICIT
        return
    for ho in HIP_OFFS:
        for f in FREQS:
            for hp in HIP_PHIS:
...
def load_done_bitmap(csv_path, n_rows):
    ...
    if not os.path.exists(csv_path):
        return bm, 0
    # With an explicit list the cell index is its position in that list, not a product
    # of axis positions -- the list is not a cross product and the two indexings differ.
    ...
    with open(csv_path) as f:
        ...
        for row in rd:
            try:
                ...
                continue
            if CELLS_EXPLICIT is not None:
                ci = _CELL_IX.get((round(float(row[0]), 4), round(float(row[1]), 4),
                                   round(float(row[2]), 4), round(float(row[3]), 4),
                                   round(float(row[4]), 4)))
                if ci is None:      # a row from some other grid sharing this file
                    continue
                gi = ci * nmu + mi
            else:
...
def check_manifest(outdir, csv_path):
    ...
    want = dict(protocol="grid6-v1", config=CONFIG, K=K,
                mujoco_version=mujoco.__version__)
...
def main():
    n_cells = (len(CELLS_EXPLICIT) if CELLS_EXPLICIT is not None else
               len(FREQS) * len(HIP_PHIS) * len(LEG_AMPS) * len(HIP_AMPS) * len(HIP_OFFS))
    ...
    if gc._MODEL.endswith("cad"):
        slide, got = 0.0, com_ratio_of(model)    # geometry already carries the ratio
        if abs(got - COM_TARGET) > 1e-3:
            raise SystemExit(f"as-built model ratio {got:.4f} != CONFIG target "
                             f"{COM_TARGET} — wrong model for CONFIG={CONFIG}")
    else:
    ...
    print(f"# GRID6 {CONFIG} model={gc._MODEL} (kappa={KAPPA} com={got:.4f} slide={slide*1000:+.2f}mm "
